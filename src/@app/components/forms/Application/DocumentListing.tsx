...
export default function DocumentListing(props: any) {
  ...
  const [tableData, refreshFn, totalRecords, extra] = useFetchApplicationDocuments({psaplref:id});
      ...
      render: (_: any, record: any) => (
        <Flex justifyContent="flex-end">
          <Space size="small">
            ...
            { detailData?.editable&&<Tooltip label='Edit' fontSize='sm'>
              <IconButton
                variant="outline"
                size={"sm"}
                borderRadius={2}
            ...
            {/* { */}
              {/* // (homeData?.access && checkAccessMatrix(homeData?.access, accessType.TRCV_DEL)) && ( */}
               { detailData?.editable&&<Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    variant="outline"
                    size={"sm"}
                    borderRadius={2}
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.psdocnme, record?.psdocnme, record?.psdocdsc)}
                  />
                </Tooltip>}
              {/* // ) */}
            {/* } */}
...
{/* {detailData?.editable && detailData?.psaplsts === "NEW" && (
              <Buttons onClick={async () => {
                await sendRequest({
                  fn: getApproveApplication({
        
                    id: mode === "EDIT" ? id : detailData?.psaplref,
                    psaplsts: "PEN",
                  }),
        ...
        <Card
              ...
              >
                <Table
                  ...
                  columns={columns}
                  data={tableData}
                  refreshFn={getApplicationDocList}
