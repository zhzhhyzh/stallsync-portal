...
import useFetchContractDocDetail from "@app/hooks/selector/useFetchContractDocDetail";
import useFetchApplicationDetail from "@app/hooks/selector/useFetchApplicationDetail";
import useFetchContractDocs from "@app/hooks/selector/useFetchContractDocs";
import { getContractDocList, getManageContractDoc } from "@app/redux/contract/slice";



export default function ContractDocumentForm(props: any) {
    ...
    const [detailData] = useFetchContractDocDetail(filename);
    ...
    async function onSubmit(data: any) {
        ...
        if (success) {
            setTimeout(async () => {
                ...
                props.onClose()
                await dispatch(getContractDocList({ psconuid: id }))
                // router.back();
            }, 200);
        }
    }
                                                        ...
                                                        (item: any) =>
                                                            ...
                                                            <Tooltip title="Download">
                                                                <DownloadOutlined
                                                                    ...
                                                                    onClick={() => handleDownload(file?.uid, file?.name)}
                                                                />
                                                            </Tooltip>
                                                        )}
