...
import useFetchMemberDetail from "@app/hooks/selector/useFetchMemberDetail";
...
const MemberProfileForm = (props: any) => {
  const { sendRequest, loading } = useApi({ title: "Customer Profile" });
  ...
  const [detailData, onInit, loading2, reset] = useFetchMemberDetail(
    id,
  );
  ...
  useEffect(() => {
    ...
    // console.log(detailData?.psaplref, " KJKJKJKJKJ")
  ...
  useEffect(() => {
    if (mode !== "ADD") {
      ...
      //   });
      if (detailData !== null && Object.keys(detailData).length > 0) formik.setValues({
        ...detailData,
  ...
  async function onSubmit(data: any) {
    data.psaplref = detailData?.psaplref
    data.psadrtyp = "REG";
    data.psadrtyph = "HOME";
    if (
        ...
        if (result2 == "success") {
          setTimeout(() => {
            showModal(dispatch, {
              ...
              message: `Application with Ref No ${resData?.application_number} has been submitted for review`,
            });
            // if(mode === "ADD")  router.back();
            // else reset()
            // router.push({
  ...
  return (
    <Box>
              ...
              <Flex direction={"column"} gap={Spacing.containerPx}>
                ...
                <Flex direction={"row"} gap={Spacing.containerPx}>
                  ...
                  <FormControl
                    id="recruitedBy"
                    // isInvalid={
                    //   Boolean(formik.errors.psaplrcb) &&
                    //   Boolean(formik.touched.psaplrcb)
                    // }
                    isReadOnly
                  >
                  ...
                  <FormControl
                    id="Company"
                    isInvalid={
                      Boolean(formik.errors.psmbrcom) &&
                      Boolean(formik.touched.psmbrcom)
                    }
                  >
                    <CustomFormLabel labelText="Company" />
                    <Select
                      isDisabled={mode === "VIEW"}                      // isDisabled={mode === "VIEW" || (!props?.profile&& detailData && Object.keys(detailData).length > 0 && !detailData?.editable)}
                      placeholder="Select Company"
                      name="psmbrcom"
                      value={formik.values.psmbrcom}
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                    >
                      {Array.isArray(companies) && companies?.map((option: any) => (
                        <option key={option.pscomuid} value={option.pscomuid}>
                          {option.pscomcde} - {option.pscomnme}
                        </option>
                      ))}
                    </Select>
                  ...
                  <Flex flex={1} flexDir="column" gap={Spacing.containerPx}>
                    ...
                    <Flex direction={"row"} gap={Spacing.containerPx}>
                      ...
                      >
                        <Flex dir="row" justifyContent="space-between">
                          <CustomFormLabel labelText="Home Address" />
                          <Checkbox
                            isDisabled={mode === "VIEW"}
                            onChange={(e: any) => {
                    ...
                    <Flex direction={"row"} gap={Spacing.containerPx}>
                      ...
                      >
                        ...
                        <Select
                          placeholder="Select State"
                          isDisabled={mode === "VIEW"} value={formik.values.psadradsh}
                          onChange={formik.handleChange}
                          onBlur={formik.handleBlur}
                        >
                          {ddlData?.STATE?.map((option: DDL_TYPES) => (
              ...
              <Flex direction={"column"} gap={Spacing.containerPx}>
                <Flex direction={"row"} gap={Spacing.containerPx}>
                  ...
                  >
                    ...
                    <Select
                      isDisabled={mode === "VIEW"} placeholder="Select Bank"
      ...
      <Flex justifyContent="flex-end" pl={10} pr={10} pt={10}>
        <Box>
          <Space size="small">
            {!props.profile && <Buttons buttonDefaultType={"BACK"} onclick={mode == "VIEW" ? () => router.push("/approvedApplication") : () => router.push("/application")} />
            }
            {mode && mode !== "VIEW" && (
            ...
            {/* {mode && mode === "EDIT" && detailData?.editable && detailData?.psaplsts === "NEW" && (
              <Buttons onClick={() => formik.setFieldValue("submitAction", "submit")}
                buttonDefaultType={"CUSTOM"} type="submit" buttonLoading={loading} buttonText="Submit" />
            )} */}
