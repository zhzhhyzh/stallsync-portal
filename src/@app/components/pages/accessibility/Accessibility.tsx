            ...
            if (!returnEmpty) {
                return (
                    <Box flexDir={'column'} alignItems={'center'} w={"60px"}>
                        <Switch
                            // <Checkbox
                            isChecked={groupCheck}
                            onChange={() =>
                                handleGroupCheckBoxOnChange(
                                    group.prfungrp,
                                    act,
                                    !groupCheck
                                )
                            ...
                            minH={'auto'}
                            colorScheme="green"
                            isReadOnly={Boolean(selectedRole)}
                        />
                        {/* <Text style={{
              color: `#f1f1f1`,
              fontSize: `.8em`,
                            ...
                            <Accordion allowMultiple minW={'600px'} pb={Spacing.gap}>
                                {(accessData || []).map((group: any) => (
                                    <AccordionItem borderRadius={0} pt={6} >
                                        ...
                                        {
                                            group.prfunctions.length > 0 ? (
                                                group.prfunctions.map((functions: any) => (
                                                    <AccordionPanel py={4} px={5} _hover={{
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <Box w={labelW}>
                                                                <Text>
                                                                    {functions.pracsfundsc}
                                                                </Text>
                                                            </Box>
                                                            <Box className="flex-1 flex flex-row gap-10">
                                                                {/* {(ddlData.FUNACT || []).map((option: DDL_TYPES) => { */}
                                                                {groupCode[group?.prfungrp]?.map((act: string) => {
                                                                    let selectedRoleArr = functions?.roles?.reduce(
                                                                        (accumulator: any[], currentValue: any) => {
                                                                            if (currentValue.pracsrol === role) {
                                                                                accumulator.push(currentValue);
                                                                    ...
                                                                    const findFuncAct = selectedRoleArr[0]?.prfunact?.find(
                                                                        (objectValue: any) => objectValue.label === act
                                                                    )
                                                                    const isChecked = findFuncAct ? findFuncAct.checked === 1 ? true : false : undefined

                                                                    // let isChecked = selectedRoleArr[0]?.prfunact
                                                                    //   ?.filter(
                                                                    //     (objectValue: any) =>
                                                                    //       objectValue.label !== "" &&
                                                                    //       objectValue.label === act
                                                                    //   )
                                                                    //   .map((roleArr: any) => {
                                                                    //     if (roleArr.label === act) {
                                                                    //       return roleArr.checked === 1 ? true : false;
                                                                    //     } else {
                                                                    //       return false;
                                                                    //     }
                                                                    //   });
                                                ...
                                                <AccordionPanel pb={2}>
                                                    <Box display={"flex"} justifyContent={"center"}>
                                                        <Text style={{ paddingLeft: 15, margin: 0 }}>
                                                            No Functions Found
                                                        </Text>
                                                    </Box>
                                                </AccordionPanel>
