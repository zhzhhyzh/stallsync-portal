...
import {
  fetchHistoriesByAccount,
} from "@app/redux/notificationSentHistory/slice";
import useFetchSentHistoriesByAccount from "@app/hooks/selector/useFetchSentHistoryByAccount";

export default function SentHistoryByAccountPage(props: any) {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const homeData = useAppSelector(selectHome);
  const { sendRequest, loading } = useApi({
    title: "Sent History By Account",
  });
  const type = "";
  ...
  const [tableData, refreshFn, totalRecords, extra] =
    useFetchSentHistoriesByAccount(accountNo);
              ...
              checkAccessMatrix(homeData?.access, accessType.NOTSNDH_VIEW) && (
                <Tooltip label="View" fontSize="sm">
                  <IconButton
                    ...
                    aria-label={"view"}
                    onClick={() =>
                      goView(record?.id, record?.psnotchn, record?.psnotcde, accountNo, cifId, "ACC")
                    }
                  />
                </Tooltip>
              )}
  ...
  return (
    <>
      <Card>
        <Text fontSize={"19pt"} fontWeight={"medium"} pb={4}>
          Sent History
        </Text>
        ...
        <Table
          ...
          columns={columns}
          data={tableData}
          refreshFn={fetchHistoriesByAccount}
