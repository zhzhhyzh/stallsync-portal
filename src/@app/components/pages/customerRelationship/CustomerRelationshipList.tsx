...
import { fetchAccountBillings, fetchAccountCustomerRelationships } from "@app/redux/accountManagement/slice";
...
import useFetchCustomerRelationships from "@app/hooks/selector/useFetchCustomerRelationships";

export default function CustomerRelationshipList(props: any) {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { sendRequest, loading } = useApi({
    title: "Customer Relationship List",
  });
  const homeData = useAppSelector(selectHome);
  const cifNo = props.cifNo;

  const { isOpen, onOpen, onClose } = useDisclosure();

  //sample code how to use this hook
  const [tableData, refreshFn, totalRecords, extra] = useFetchCustomerRelationships();
    ...
    <>
      <Card>
        ...
        <Table
          columns={columns}
          data={tableData}
          refreshFn={fetchAccountCustomerRelationships}
        ...
        <ModalContent w={"150%"}>
          <ModalHeader>
            <Text fontSize={"3xl"} fontWeight="normal" mb={1}>
              Customer Relationship Details - Edit
            </Text>
          </ModalHeader>
          <ModalCloseButton />
          <ModalBody>
