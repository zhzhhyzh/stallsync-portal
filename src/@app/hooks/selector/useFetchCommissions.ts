...
import {selectExtra, selectGcs, getGroupCom, selectGcsTotal, selectCommissions, selectTotal, getCom } from "@app/redux/commission/slice";

function useFetchCommissions() {
  const dispatch = useAppDispatch();
  const router = useRouter();

  const data = useAppSelector(selectCommissions);
