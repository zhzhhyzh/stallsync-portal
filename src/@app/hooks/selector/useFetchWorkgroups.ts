...
function useFetchWorkgroups() {
  const dispatch = useAppDispatch();
  const router = useRouter();

  const data = useAppSelector(selectWorkgroups);
  const total = useAppSelector(selectWorkgroupTotal)
  ...
  async function onInit({ page, limit }: useFetchWorkgroupsProps) {
    setLoading(true);
...
interface useFetchWorkgroupsProps {
  page?: number;
  limit?: number;
//   functionCode?: string; 
}
