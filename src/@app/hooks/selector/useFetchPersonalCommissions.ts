...
import {selectExtra, selectPcs, getPersonalCom, selectPcsTotal } from "@app/redux/commission/slice";

function useFetchPersonalCommissions() {
  const dispatch = useAppDispatch();
  const router = useRouter();

  const data = useAppSelector(selectPcs);
  const total = useAppSelector(selectPcsTotal);
...
export default useFetchPersonalCommissions;
