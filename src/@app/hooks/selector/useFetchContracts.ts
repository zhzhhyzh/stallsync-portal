...
import {selectExtra, selectContracts, selectContractsTotal, getContractList } from "@app/redux/contract/slice";

function useFetchContracts() {
  const dispatch = useAppDispatch();
  const router = useRouter();

  const data = useAppSelector(selectContracts);
  const total = useAppSelector(selectContractsTotal)
