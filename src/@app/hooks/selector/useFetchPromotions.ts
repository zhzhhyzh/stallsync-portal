...
import {selectExtra, selectPromotions, selectTotal, getPromotionList } from "@app/redux/promotion/slice";

function useFetchPromotions() {
  const dispatch = useAppDispatch();
  const router = useRouter();

  const data = useAppSelector(selectPromotions);
