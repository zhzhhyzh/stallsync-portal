...
import useFetchCompars from "@app/hooks/selector/useFetchCompars";
import {
  fetchCompar,
  getremoveCompar,
} from "@app/redux/compar/slice";
...
export default function ComparPage() {
  ...
  const { sendRequest, loading } = useApi({ title: "Company Parameter" });
  const homeData = useAppSelector(selectHome);

  //sample code how to use this hook
  const [tableData, refreshFn, totalRecords, extra] = useFetchCompars({});
  ...
  const columns: any[] = [
    ...
    },
    {
      ...
      key: "pscomspv",
      render(_: any, _f: any) {
        return <Text>{_f.pscomspvdsc}</Text>
      }
    },
    {
      ...
      render: (data: any, record: any) => (
        <Flex sx={{
          ...
          {data === "Y" ? <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
            ...
            borderRadius: 10
          }}>{record?.pscomstsdsc}</Text> : <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
            width: 60,
            height: 20,
            backgroundColor: Colors.DANGER,
            borderRadius: 10,
          }}>{record?.pscomstsdsc}</Text>}
              ...
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.TAXPARAM_DEL)) && (
                <Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.id, record?.pscomcde)}
                  />
                </Tooltip>
              )
            }
    ...
    <>
      <Box>
        <Flex justifyContent={"space-between"} pl={4} pr={4} pt={4}>
          ...
          >
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.COMPAR_ADD)) && (
                <Buttons
