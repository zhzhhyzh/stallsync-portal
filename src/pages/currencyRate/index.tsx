...
import useFetchCurrencyRates from "@app/hooks/selector/useFetchCurrencyRates";
import {
  fetchCurrat,
  getRemoveCurrat,
} from "@app/redux/currencyRate/slice";
...
export default function CURRATPage() {
  ...
  const [tableData, refreshFn, totalRecords, extra] = useFetchCurrencyRates({});
  ...
  const columns: any[] = [
    {
      ...
      key: "pscurcde",
      render(_: any, _f: any) {
        return <Text>{_f.pscurcde} - {_f.pscurcdedsc}</Text>
      }
    },
    {
      title: "Currency Rate",
      ...
      render: (data: any, record: any) => (
        <Flex sx={{
          ...
          {data === "Y" ? <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
            ...
            borderRadius: 10
          }}>{record?.pscurstsdsc}</Text> : <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
            width: 60,
            height: 20,
            backgroundColor: Colors.DANGER,
            borderRadius: 10,
          }}>{record?.pscurstsdsc}</Text>}
      ...
      render: (_: any, record: any) => (
        <Flex justifyContent="flex-end">
          <Space size="small">
            ...
            /> */}
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.TAXPARAM_EDIT)) && (
            ...
            {
              // todo
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.TAXPARAM_DEL)) && (
                <Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.id, record?.pscurrat)}
                  />
                </Tooltip>
              )
            }
    ...
    <>
      <Box>
        <Flex justifyContent={"space-between"} pl={4} pr={4} pt={4}>
          ...
          >
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.CURRAT_ADD)) && (
                <Buttons
