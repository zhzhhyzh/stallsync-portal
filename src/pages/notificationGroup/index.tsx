...
import useFetchNotificationGroupList from "@app/hooks/selector/useFetchNotificationGroupList";

import {
    getNotificationGroupList,
    getRemoveNotificationGroup
} from "@app/redux/notificationGroup/slice";
...
export default function NotificationGroupPage() {
  ...
  const [tableData, refreshFn, totalRecords, extra] = useFetchNotificationGroupList();
  const [ddlData] = useFetchDDL({ code: ["GTCAT", "NOTCATG", "YESORNO"] });
        ...
        render: (data: any, record: any) => (
          <Flex sx={{
            ...
            {data === "Y" ? <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
              ...
              borderRadius: 10
            }}>{record?.psngpstsdsc}</Text> : <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
              width: 60,
              height: 20,
              backgroundColor: Colors.DANGER,
              borderRadius: 10,
            }}>{record?.psngpstsdsc}</Text>}
      ...
      render: (_: any, record: any) => (
        <Flex justifyContent="center">
          <Space size="small">
              ...
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.NOTGRP_DEL)) && (
                <Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.id, record?.psngpdsc)}
                  />
                </Tooltip>
              )
            }
            ...
            <TableMenu menus={[
             (homeData?.access && checkAccessMatrix(homeData?.access, accessType.NOTGRP_EDIT)) && {
                url: `/notificationGroup/notificationGroupData`,
                query: {
                  // id: record?.id,
                  psngpcde: record?.psngpcde,
                  psngpdsc: record?.psngpdsc,
                  // file: extra.file
                },
                label: "Notification Group Data"
              },
              {
                  url: `/maintLogs`,
                  query: {
  ...
  async function onRemove(id: string) {
    const { success } = await sendRequest({
      fn: getRemoveNotificationGroup({ id }),
    });
    if (success) {
  ...
  function goAdd() {
    router.push({
      pathname: "/notificationGroup/Detail",
      query: {
        id: "",
        mode: "ADD"
      },
      ...
      <Box>
        ...
        <Card p={1} mt={4}>
          ...
          <Table
            ...
            columns={columns}
            data={tableData}
            refreshFn={getNotificationGroupList}
