...
import useFetchDsagents from "@app/hooks/selector/useFetchDsagents";
import {
  fetchDsagent,
  getremoveDsagent,
} from "@app/redux/dsagent/slice";
...
export default function DsagentPage() {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { sendRequest, loading } = useApi({ title: "Designation Agent" });
  const homeData = useAppSelector(selectHome);

  //sample code how to use this hook
  const [tableData, refreshFn, totalRecords, extra] = useFetchDsagents({});
  ...
  const columns: any[] = [
    ...
    },
    {
      ...
      render: (data: any, record: any) => {
        return record.psdsgpre ? (
          <Text>{`${record.psdsgpre}`}</Text>
        ) : (
      ...
      render: (data: any, record: any) => (
        <Flex sx={{
          ...
          {data === "Y" ? <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
            ...
            borderRadius: 10
          }}>{record?.psdsgstsdsc}</Text> : <Text fontWeight={"normal"} color={"white"} textAlign="center" style={{
            width: 60,
            height: 20,
            backgroundColor: Colors.DANGER,
            borderRadius: 10,
          }}>{record?.psdsgstsdsc}</Text>}
      ...
      render: (_: any, record: any) => (
        <Flex justifyContent="flex-end">
          <Space size="small">
            ...
            /> */}
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.DSAGENT_EDIT)) && (
            ...
            {
              // todo
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.DSAGENT_DEL)) && (
    ...
    <>
      <Box>
        <Flex justifyContent={"space-between"} pl={4} pr={4} pt={4}>
          ...
          >
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.DSAGENT_ADD)) && (
                <Buttons
