...
import useFetchFeeCodes from "@app/hooks/selector/useFetchFeeCodes";
import {
  fetchFeeCodes,
  getRemoveFeeCode,
} from "@app/redux/feeCode/slice";
...
export default function FeeCodePage() {
  ...
  const [tableData, refreshFn, totalRecords, extra] = useFetchFeeCodes({});
      ...
      render: (_: any, record: any) => (
        <div style={customDataCellStyle}>
          <Space size="small">
            ...
            /> */}
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.FEECODE_EDIT)) && (
            ...
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.FEECODE_DEL)) && (
                <Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.id, record?.psfeedsc)}
                  />
                </Tooltip>
              )
            }
            ...
            <TableMenu menus={[
             
              {
                  ...
                  breadcrumbRoute: [
                    {
                      title: "Fee Code",
                      href: `/feeCodes`,// Add parameter if needed eg. /generalParameter/?id=123
                    },
                    {
              ...
              // {
              //   url: `/feeCodes/productQueues`,
              //   query: {
              //     // id: record?.id,
              //     prquecod: prquecod,
              //     prprodcd: record?.psfeecod,
              //     // file: extra.file
              //   },
              //   label: "Product Queue"
              // },
              // {
              //   url: `/feeCodes/productFees`, //new subfolder place inside feeCodes
              //   query: {
              //     // id: record?.id,
              //     prquecod: prquecod,
              //     prprodcd: record?.psfeecod,
    ...
    <>
      <Box>
        <Flex justifyContent={"space-between"} pl={4} pr={4} pt={4} py={Spacing.gap}>
          ...
          >
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.FEECODE_ADD)) && (
                <Buttons 
