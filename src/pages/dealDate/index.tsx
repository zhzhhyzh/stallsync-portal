...
import useFetchDealDates from "@app/hooks/selector/useFetchDealDates";
import {
  fetchDealDate,
  getRemoveDealDate,
} from "@app/redux/dealDate/slice";
...
export default function dealDatePage() {
  ...
  const [tableData, refreshFn, totalRecords, extra] = useFetchDealDates({});
  ...
  function yearOnChange(event: any) {
  ...
  const columns: any[] = [
    ...
    {
      title: "Closing Date 1",
      dataIndex: "closing_date1",
      key: "closing_date1",
      
    },
    {
      title: "Closing Date 2",
      dataIndex: "closing_date2",
      key: "closing_date2",
      ...
      render: (_: any, record: any) => (
        <Flex justifyContent="flex-end">
          <Space size="small">
            ...
            /> */}
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.DEAL_DATE_EDIT)) && (
            ...
            {
              // todo
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.DEAL_DATE_DEL)) && (
                <Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.id, record?.processing_month)}
                  />
                </Tooltip>
              )
            }
    ...
    <>
      <Box>
        <Flex justifyContent={"space-between"} pl={4} pr={4} pt={4}>
          ...
          >
            {
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.DEAL_DATE_ADD)) && (
                <Buttons
