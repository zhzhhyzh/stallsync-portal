...
import useFetchTaxCodes from "@app/hooks/selector/useFetchTaxCodes";
import {
  fetchTaxCodes,
  getRemoveTaxCode,
} from "@app/redux/taxCode/slice";
...
export default function TranCodePage() {
  ...
  const [tableData, refreshFn, totalRecords, extra] = useFetchTaxCodes({});
              ...
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.TAXPARAM_DEL)) && (
                <Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.id, record?.pstaxdsc)}
                  />
                </Tooltip>
              )
            }
