...
import useFetchChannelCodeList from "@app/hooks/selector/useFetchChannelCodeList";

import {
    getChannelCodeList,
    getRemoveChannelCode
} from "@app/redux/channelCode/slice";
...
export default function CommunicationChannelPage() {
  ...
  const [tableData, refreshFn, totalRecords, extra] = useFetchChannelCodeList();
  ...
  const columns: any[] = [
    ...
    },
    {
      ...
      render: (data: any, record: any) => (
          ...
          // <Space size={"small"}>
           <span>{data}-{record.pschntypdsc}</span> 
          // </Space>
          ...
          <Space size="small">
              ...
              (homeData?.access && checkAccessMatrix(homeData?.access, accessType.CHNLCOM_DEL)) && (
                <Tooltip label='Delete' fontSize='sm'>
                  <IconButton
                    ...
                    icon={<IoTrash />}
                    aria-label={"delete"}
                    onClick={() => alertRemove(record?.id, record?.pschncde, record?.pschndsc)}
                  />
                </Tooltip>
              )
            }
            ...
            <TableMenu menus={[
             (homeData?.access && checkAccessMatrix(homeData?.access, accessType.CHNLCOM_VIEW)) && {
                url: `/channelCode/notificationCategory`,
                query: {
                  id: record?.id,
                  pschncde: record?.pschncde,
                  // file: extra.file
                },
            ...
            >
              <Space size="middle">
                ...
                <Select
                  ...
                  value={category}
                >
                  {ddlData?.CHNTYPE?.map((option: DDL_TYPES) => (
